//  主调用函数是 setdate(this,[object])和setday(this)，[object]是控件输出的控件名，举两个例子：
//  一、<input name=txt><input type=button value=setday onclick="setday(this,document.all.txt)">
//  二、<input onfocus="setdate(this)">
var bMoveable=true;
var strFrame;  
document.writeln('<iframe id=endDateLayer frameborder=0 width=160 height=167 bgcolor="#F7F7F7" style="position: absolute;  z-index: 9998; display: none"></iframe>');
strFrame='<style>';
...
strFrame+='<table border=0 cellspacing=1 cellpadding=0 width=142 height=160 bordercolor=#63A3E9 bgcolor=#CADEFF >';
strFrame+='    <tr><td width=142 height=23  bgcolor=#E0ECFF>';
strFrame+='        <table border=0 cellspacing=0 cellpadding=0 width=158  height=23>';
strFrame+='            <tr align=center >';
strFrame+='                <td width=16 align=center bgcolor=#E0ECFF style="font-size:12px;cursor: hand;color: #000000" ';
strFrame+='        onclick="parent.meizzPrevM()" title="向前翻 1 月" ><b >&lt;</b></td>';
strFrame+='       <td width=60 align="right" bgcolor="#E0ECFF"  style="font-size:12px;cursor:hand" ';
strFrame+='           onmouseover="style.backgroundColor=\'#aaccf3\'"';
strFrame+='        onmouseout="style.backgroundColor=\'#E0ECFF\'" ';
...
strFrame+='       <td width=48 align="left" style="font-size:12px;font-color: #ffffff;cursor:hand" ';
strFrame+='        bgcolor="#E0ECFF" onmouseover="style.backgroundColor=\'#aaccf3\'" ';
strFrame+='        onmouseout="style.backgroundColor=\'#E0ECFF\'" ';
...
strFrame+='       <td width=16 bgcolor=#E0ECFF align=center style="font-size:12px;cursor: hand;color: #000000" ';
strFrame+='        onclick="parent.meizzNextM()" title="向后翻 1 月" ><b >&gt;</b></td>';
strFrame+='      </tr>';
strFrame+='     </table></td></tr>';
strFrame+='    <tr><td width=142 height=18 bgcolor=#F5F5F5 >';
strFrame+='     <table border=0 cellspacing=0 cellpadding=2 bgcolor=#F5F5F5 ' + (bMoveable? 'onmousedown="DragStart()" onmouseup="DragEnd()"':'');
strFrame+='    width=142 height=20  style="cursor:' + (bMoveable ? 'move':'default') + '">';
strFrame+='    <tr><td style="font-size:12px;color:#000000" width=20>&nbsp;日</td>';
strFrame+='<td style="font-size:12px;color:#000000" >&nbsp;一</td><td style="font-size:12px;color:#000000">&nbsp;二</td>';
strFrame+='<td style="font-size:12px;color:#000000" >&nbsp;三</td><td style="font-size:12px;color:#000000" >&nbsp;四</td>';
strFrame+='<td style="font-size:12px;color:#000000" >&nbsp;五</td><td style="font-size:12px;color:#000000" >&nbsp;六</td></tr>';
strFrame+='</table></td></tr>';
strFrame+='  <tr ><td width=142 height=120 >';
strFrame+='    <table border=0 cellspacing=0 cellpadding=2 BORDERCOLORLIGHT=#63A3E9 BORDERCOLORDARK=#FFFFFF bgcolor=#ffffff width=158 height=120 >';
	var n=0; for (j=0;j<5;j++){ 
strFrame+= ' <tr align=center >'; 
						for (i=0;i<7;i++){
strFrame+='<td width=20 height=20 id=meizzDay'+n+' style="font-size:12px;'
						if (i==0 ) {
strFrame+='color:#CC0000" '}
						else if (i==6 ) {
strFrame+='color:#36A136" '}
						else {
strFrame+='color:#777777" '}
strFrame+='onclick=parent.meizzDayClick(this.innerText,0)></td>';
						n++;}
strFrame+='</tr>';}
strFrame+='      <tr align=center >';
...
strFrame+='         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="当前日期时间">今天</span>&nbsp;&nbsp;<span style="cursor:hand" id=evaAllOK onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'"  onclick="parent.closeLayer()" title="关闭日历">关闭&nbsp;</span></td></tr>';
...
function meizzToday() // Today Button
{
 ...
 if(outObject)
 {
  ...
  {
  	/*
	 * outObject.value= parent.meizzTheYear + "-" + format(
	 * parent.meizzTheMonth) + "-" + format(parent.meizzTheDate) + " " +
	 * format(parent.meizzTheHour) + ":" + format(parent.meizzTheMinute) + ":" +
...
function meizzSetDay(yy,mm) // 主要的写程序**********
{
 ...
 {
  // 初始化边框
  da.borderColorLight="#F5F5F5";
  da.borderColorDark="#F5F5F5";
  ...
  if(i<firstday)  // 上个月的部分
  {
   ...
   else
   {
    ...
    meizzWDay[i] == new Date().getDate()) ? "#5CEFA0":"#ffffff");
    // 将选中的日期显示为凹下去
...
{
 /*
	 * if (bUseTime) { outObject.value= yy + "-" + format(mm) + "-" + format(n) + " " +
	 * format(hh) + ":" + format(mi) + ":" + format(se); //注：在这里你可以输出改成你想要的格式 }
	 * else { outObject.value= yy + "-" + format(mm) + "-" + format(n);
	 * //注：在这里你可以输出改成你想要的格式 }
	 */
